...
export const MisionesModal: React.FC<MisionesModalProps> = ({
  ...
  // FULL SCREEN VIEW RENDERING (When accessed from main navigation or with onBack)
  if (onBack) {
    return (
      <div className="misiones-screen-view">
        ...
        {/* Screen Header */}
        <div className="misiones-screen-header">
          <button className="misiones-screen-back-btn" onClick={onBack} title="Volver al Menú Principal">
            <span>⬅</span>
            <span>Volver al Menú</span>
          </button>

          <div className="misiones-screen-title-box">
            <span style={{ fontSize: '1.8rem' }}>📜</span>
            <h2>MISIONES Y RECOMPENSAS</h2>
          </div>

          <div className="misiones-screen-balances">
            <div className="misiones-screen-balance-tag" title="Monedas de Oro">
              🪙 {userGold.toLocaleString()} Oro
            </div>
            <div className="misiones-screen-balance-tag" title="Gemas Disponibles">
        ...
        {/* Feedback banners */}
        {errorMessage && (
          <div style={{ background: 'rgba(239, 68, 68, 0.2)', border: '1px solid #ef4444', color: '#fca5a5', padding: '8px 16px', margin: '8px 0', borderRadius: '8px', fontSize: '0.85rem' }}>
            ⚠️ {errorMessage}
          </div>
        )}
        {successMessage && (
          <div style={{ background: 'rgba(16, 185, 129, 0.2)', border: '1px solid #10b981', color: '#6ee7b7', padding: '8px 16px', margin: '8px 0', borderRadius: '8px', fontSize: '0.85rem' }}>
            🎉 {successMessage}
          </div>
        )}

        {/* Content Body */}
        <div className="misiones-content" style={{ padding: '6px 0' }}>
          {loading ? (
            ...
            <>
              {/* TAB 1: 3-COLUMN FULL SCREEN LAYOUT */}
              {activeTab === 'diarias' && (
                <div className="misiones-3cols-layout">
                  {/* COLUMNA 1: PROGRESO SEMANAL Y COFRES */}
                  <div className="misiones-col--weekly">
                    <div className="weekly-col-header">
                      <div className="weekly-col-title">Progreso Semanal de Puntos</div>
                      <div className="weekly-col-subtitle">Se reinicia cada lunes a las 00:00 UTC</div>
                    </div>

                    <div className="weekly-col-pts-badge">
                      ⭐ {weeklyPts} / {maxWeeklyPts} Pts
                    </div>

                    <div className="chests-bar-bg" style={{ margin: '4px 0' }}>
                      <div className="chests-bar-fill" style={{ width: `${weeklyProgressPercent}%` }} />
                    </div>

                    <div className="weekly-col-chests-list">
                      {/* Bronze Chest: 70 Pts */}
                      {(() => {
                        ...
                        return (
                          <div className={`weekly-col-chest-item ${weeklyPts >= 70 ? 'weekly-col-chest-item--unlocked' : ''} ${isClaimed ? 'weekly-col-chest-item--claimed' : ''}`}>
                            <div className="weekly-col-chest-info">
                              <div className="weekly-col-chest-icon">📦</div>
                            ...
                            {isClaimable ? (
                              ...
                              <span style={{ fontSize: '0.72rem', color: '#10b981', fontWeight: 800 }}>✓ Listo</span>
                            ) : (
                              <span style={{ fontSize: '0.72rem', color: '#64748b', fontWeight: 700 }}>🔒 {70 - weeklyPts} pts</span>
                            )}
                          </div>
                        )
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <div className={`weekly-col-chest-item ${weeklyPts >= 140 ? 'weekly-col-chest-item--unlocked' : ''} ${isClaimed ? 'weekly-col-chest-item--claimed' : ''}`}>
                            <div className="weekly-col-chest-info">
                              <div className="weekly-col-chest-icon">🥈</div>
                              <div className="weekly-col-chest-text">
                            ...
                            {isClaimable ? (
                              ...
                              <span style={{ fontSize: '0.72rem', color: '#64748b', fontWeight: 700 }}>🔒 {140 - weeklyPts} pts</span>
                            )}
                          </div>
                        )
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <div className={`weekly-col-chest-item ${weeklyPts >= 210 ? 'weekly-col-chest-item--unlocked' : ''} ${isClaimed ? 'weekly-col-chest-item--claimed' : ''}`}>
                            <div className="weekly-col-chest-info">
                              <div className="weekly-col-chest-icon">🏆</div>
                              <div className="weekly-col-chest-text">
                                <span className="weekly-col-chest-name">Cofre Dorado</span>
                                <span className="weekly-col-chest-pts">210 Pts Requeridos</span>
                                <span className="weekly-col-chest-desc">1 Sobre Épico (Místico)</span>
                              </div>
                            </div>
                            {isClaimable ? (
                              <button
                              ...
                              <span style={{ fontSize: '0.72rem', color: '#64748b', fontWeight: 700 }}>🔒 {210 - weeklyPts} pts</span>
                            )}
                          </div>
                        )
                      })()}
              ...
              {/* TAB 2: RACHA 7 DÍAS */}
              {activeTab === 'racha' && renderRachaTab()}

              {/* TAB 3: CONCURSO TIKTOK */}
        ...
        {rerollSlot !== null && renderRerollModal()}
      </div>
    )
  }

  // MODAL FALLBACK VIEW (When opened as a popup without onBack)
  return createPortal(
